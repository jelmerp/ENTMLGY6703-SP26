...
1. **Sample metadata** -- accessed with `colData()`\
   A table with the key sample grouping (here: treatment, time) information.
...
Let's see what's inside this DESeq2 object:
...
The sample metadata can be accessed with the `colData()` function,
...
The gene count table is stored in the DESeq2 object as well,
which you can access with the `counts()` function:
...
Only the first 6 rows (=genes) are printed, but because the table is so wide,
...
The total number of genes counts per sample is called the **library size**.
_TBA - EXPAND_
...
::: exercise
#### {{< fa user-edit >}} Exercise: Interpret the library sizes

If one sample has 9 million reads and another 28 million reads,
what implications does this have for comparing gene expression levels?
:::
...
- `sum()` the number of times these total counts are greater than 0
...
1. **Gene count normalization**\
   To fairly compare samples, raw gene counts must be adjusted:

   - By **library size**, the total number of gene counts per sample
   - By **library composition**,
     e.g. to correct for extremely abundant genes that "steal" most of a
     sample's counts
