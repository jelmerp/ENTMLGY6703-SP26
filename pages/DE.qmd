...
- The `time` column contains the amount of time after infection,
  where "h" is short for hours.
...
We'll now have to make some modifications to the metadata table
so that it can be used in DESeq2:

- The table should be sorted by sample ID: we can use the `arrange()` function to do so
...
pca_df |>
  mutate(
    time = factor(time, levels = c("24h", "10d")),
    treatment = factor(treatment, levels = c("control", "relictum", "cathemerium"))
    ) |>
  ggplot(aes(x = PC1, y = PC2, color = treatment, shape = time)) +
  geom_point(size = 5) +
  labs(
...
Given the PCA results, do you think we can ignore the `time` variable?
...
- An analysis with two independent variables (factors): `~ time + treatment`
...
You will now create a new variable that is a combination of `treatment` and `time`,
and call it `group`:

```{r}
# Create a combined variable called 'group':
group <- paste(dds$treatment, dds$time, sep = "_")
...
2. The first (reference) level of the independent variable (in the example below, `relictum_24h`)
3. The second level of the independent variable (in the example below, `control_24h`)

```{r}
focal_contrast <- c("group", "relictum_24h", "control_24h")
...
This means that you found `r sum(res_rc24$padj < 0.05, na.rm = TRUE)`
...
top_df <- data.frame(res_rc24) |> arrange(padj)
...
```{r, eval=FALSE}
focal_gene_counts |>
  # By converting 'time' and 'treatment' columns into "factors",
...
```{r, eval=FALSE}
# Write the 
write_tsv(as.data.frame(res_rc24), "DE-results_rc24.tsv")
```
