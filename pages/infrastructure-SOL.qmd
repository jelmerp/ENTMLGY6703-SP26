...
Access to OSC storage space and compute nodes is managed through **projects**.
For this course, we have a project called `PAS2880`, which you have been added to.
...
1. **Home Directory** --
   for miscellaneous personal files
2. `/fs/scratch/PAS2880` --
   the "**scratch**" folder for temporary storage for project `PAS2880`
3. `/fs/ess/PAS2880` --
   the "**project**" folder for permanent storage for project `PAS2880`

Click on the `PAS2880` scratch folder, `/fs/scratch/PAS2880`:
you should see the folders and files that are present there, 
...
![_The OnDemand file browser_](../img/ondemand_files_PAS2880_2.png){fig-align="center" width="95%" fig-alt="A screenshot of the OSC OnDemand file browser." .lightbox}
...
We won't use any of these today, but in next week's homework and lab,
you will use RStudio ("RStudio Server") from this menu.
...
::: callout-note
### And there are other good reasons to use the shell, including:
...
![](../img/ondemand_shell3.png){fig-align="center" width="95%" fig-alt="A screenshot of the OSC OnDemand shell interface." .lightbox}
...
- I will go ahead and _clear the screen_, so the prompt is nicely at the top,
...
- The tilde `~` is shorthand for a user's home folder
- You may see a different node name,
  depending on which login node you happened to be connected to

``` bash-in-nocolor
[jelmer@cardinal-login01 ~]$ 
```

The shell is used by typing a command **after the dollar sign** `$`,
and then pressing <kbd>Enter</kbd> to execute it.
...
_In the live session, I will demonstrate a neat feature called "tab completion"!_
:::
...
But let's also see what happens when it does not succeed — an error appears:
...
<details><summary>{{< fa comments >}} What was the problem with the path we specified? Does that surprise you? *(Click to see the solution)*</summary>
...
<details><summary>{{< fa comments >}} What is different about the output, and what do you think that means? *(Click to see the solution)*</summary>
